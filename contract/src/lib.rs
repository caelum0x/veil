...
const DENOM_KEY: Symbol = symbol_short!("denom");

// Default denomination (1 XLM in stroops), used if a pool is deployed without
// an explicit denomination for backwards compatibility.
const DEFAULT_DENOM: i128 = 1000000000;

/// Encodes a non-negative `i128` denomination as the 32-byte big-endian field
...
impl PrivacyPoolsContract {
    pub fn __constructor(
        ...
        // Per-pool fixed denomination. Each denomination is its own pool/instance
        // (1, 10, 100, 1000 ...). A non-positive value falls back to the default.
    ...
    pub fn deposit(env: &Env, from: Address, commitment: BytesN<32>) -> Result<u32, Error> {
        ...
        // Create token client and transfer this pool's denomination from depositor to contract
        let denom = Self::get_denom(env);
    ...
    pub fn withdraw(
        ...
        let withdrawn_value = &pub_signals.pub_signals.get(1).unwrap();
        let proof_root = &pub_signals.pub_signals.get(2).unwrap();
        let proof_association_root = &pub_signals.pub_signals.get(3).unwrap();

        // Bind the proven withdrawal amount to this pool's denomination. The circuit
        // range-proves `withdrawnValue <= value`, so enforcing equality here means a
        // depositor can never withdraw more than the denomination they funded: a coin
        // with value < denom can never produce a valid proof (range check fails), and a
        // coin with value > denom can only ever release `denom`. This is what makes a
        // fixed-denomination pool sound without inspecting the hidden commitment value.
        let expected_value = denom_to_signal_bytes(env, denom);
        if withdrawn_value.to_bytes() != expected_value {
            return vec![env, String::from_str(env, "Withdrawn amount does not match pool denomination")];
        }
    ...
    /// Gets this pool's fixed denomination (in stroops of the configured token).
    pub fn get_denom(env: &Env) -> i128 {
        env.storage()
            .instance()
            .get(&DENOM_KEY)
