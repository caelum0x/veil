...
impl WithdrawalManager {
    ...
    pub fn withdraw_coin(
        ...
        Ok(SnarkInput {
            // Full withdrawal: withdraw exactly the coin's denomination. The contract
            // enforces `withdrawnValue == pool denom`, so this must be the coin's own
            // value rather than a hardcoded constant (which broke non-default pools).
