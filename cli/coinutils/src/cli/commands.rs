...
impl CommandHandler {
    ...
    pub fn handle_generate(&self, scope: String, output: String, value: i128) -> Result<()> {
        info!("Generating coin with scope: {} (value: {} stroops)", scope, value);
        debug!("Output file: {}", output);

        let env = Env::default();
        env.cost_estimate().budget().reset_unlimited();

        let generated_coin = generate_coin(&env, scope.as_bytes(), value);
        debug!(
